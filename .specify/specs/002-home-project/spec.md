...
The CV shall be provided as a static PDF asset at:

```text
src/assets/documents/alejandro-sierra-cv.pdf
```

The "Download CV" action shall reference this asset using a base-href-compatible
path so it works correctly under the GitHub Pages deployment path:
```
/portfolio/
```

No backend endpoint, API, CMS, dynamic document generation, or external
document hosting service shall be introduced for the CV.

The final production build shall make the document available at:
```
/portfolio/assets/documents/alejandro-sierra-cv.pdf
```
### 5.3 Visual
...
For future features that are not yet implemented, navigation actions shall
point to the corresponding available section on the Home page when meaningful.

For this feature:

- "Explore my work" shall scroll to `#selected-work`.
- "View projects" shall scroll to `#selected-work`.
- "Download CV" shall point to the available CV document.
- GitHub shall point to the real GitHub profile URL.
- LinkedIn shall point to the real LinkedIn profile URL.

No fake routes, "coming soon" pages, or dead links shall be created.

When the future Projects feature is implemented, the Home navigation may be
updated to point directly to the Projects route.
